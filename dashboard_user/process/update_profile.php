...
if (isset($_FILES['ktp']) && $_FILES['ktp']['error'] == UPLOAD_ERR_OK) {
  $uploadDir = '../../dashboard_admin/assets/img/ktp/';
  if (!is_dir($uploadDir)) {
...
$query = "UPDATE pengguna SET nama_pengguna = ?, email = ?, nomor_telepon = ?, alamat = ?, ktp = ? WHERE id = ?";
$stmt = mysqli_prepare($connection, $query);
mysqli_stmt_bind_param($stmt, "sssssi", $nama, $email, $nomor_telepon, $alamat, $ktp_path, $id_user);
...
if ($success) {
  ...
  $_SESSION['login']['nomor_telepon'] = $nomor_telepon;
